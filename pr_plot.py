...
for temp, (model, step) in enumerate(zip(test_model, test_epoch)):
    y_scores = np.load(path1+model+'_all_prob' + '_' + step + '.npy')
    y_true = np.load(path1+model+'_all_label' + '_' + step + '.npy')
    ...
    plt.plot(recall[:], precision[:], lw=2, color=color[0],label="CNN+KGATT")

for temp, (model, step) in enumerate(zip(test_model, test_epoch)):
    y_scores = np.load(path2+model+'_all_prob' + '_' + step + '.npy')
    y_true = np.load(path2+model+'_all_label' + '_' + step + '.npy')
    y_scores = np.reshape(y_scores,(-1))
    y_true = np.reshape(y_true,(-1))
    precision,recall,threshold = precision_recall_curve(y_true,y_scores)
    p,r,i = PrecisionAtRecall(precision, recall, 0.1)
    ...
    print('precison: {}, recall: {}'.format(p, r))
    average_precision = average_precision_score(y_true, y_scores)
    avg_pres_desc                                   .append(average_precision)
    recall = recall[::-1]
    precision = precision[::-1]
    plt.plot(recall[:], precision[:], lw=2, color=color[1],label="CNN+KGATT+Desc")
